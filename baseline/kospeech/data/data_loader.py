...
def split_dataset(config: DictConfig, transcripts_path: str, vocab: Vocabulary, valid_size=0.3, random_state=42):
    """
    ...
    train_audio_paths, valid_audio_paths, train_transcripts, valid_transcripts = train_test_split(audio_paths, transcripts, test_size=valid_size, random_state=random_state)

    train_num = len(train_audio_paths)
    valid_num = len(valid_audio_paths)

    total_time_step = math.ceil( (train_num + valid_num) / config.train.batch_size)
    ...
    valid_num_per_worker = math.ceil(valid_num / config.train.num_workers)

    # seperating the valid dataset by the number of workers
    for idx in range(config.train.num_workers):
        valid_begin_idx = valid_num_per_worker * idx
        valid_end_idx = min(valid_num_per_worker * (idx + 1), valid_num)

        validset_list.append(
            SpectrogramDataset(
                valid_audio_paths[valid_begin_idx:valid_end_idx],
                valid_transcripts[valid_begin_idx:valid_end_idx],
                vocab.sos_id, vocab.eos_id,
                config=config,
                dataset_path=config.train.dataset_path,
                audio_extension=config.audio.audio_extension,
    ...
    return train_time_step, valid_time_step, trainset_list, validset_list
