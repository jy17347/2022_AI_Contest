...
def generate_character_labels(transcripts, labels_dest, vocab_size=2000):
    ...
    label_df.to_csv(os.path.join(labels_dest, "labels.csv"), encoding="utf-8", index=False)


def generate_character_script(audio_paths, transcripts, labels_dest, save_path, mode):
    print('create_script started..')
    char2id, id2char = load_label(os.path.join(labels_dest, "labels.csv"))

    with open(os.path.join(save_path, f"{mode}_transcript.txt"), "w") as f:
